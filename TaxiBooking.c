...
int main()
{
    ...
    while(1)
    {
        ...
        printf("\nWelcome to Kaufman Cabs. Please select an option from the menu below:\n");
        printf("+ 1.Booking\n+ 2.Details\n+ 3.Exit\n");
  scanf("%d",&user_choice);
        if(user_choice==1)
        {
            printf("\nInput no of passengers travelling today along with you:\n");
            scanf("%d",&count);
            for(i=0;i<count;i++)
            {
                printf("\nEnter the following details below to confirm your ride! %d:\n",i+1);
                printf("\nCustomer Id:\n");
                ...
                {
                 printf("all taxis are busy atm, please try again later.");
                }
                decrement();
            }
        }
